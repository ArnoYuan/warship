...
#define SRAM_BASE_ADDRESS   ((uint32_t)(0x68000000))

void sram_write(uint32_t address, uint8_t* wr_data, size_t wr_len)
{
  for(int i=0; i < wr_len; i++)
  {
    *(pie_volatile uint8_t*)(SRAM_BASE_ADDRESS + address) = *wr_data;
    address += 1;
    wr_data ++;
  }
...
void sram_read(uint32_t address, uint8_t* rd_data, size_t rd_len)
{
  for(int i=0; i < rd_len; i ++)
  {
    *rd_data = *(pie_volatile uint8_t*)(SRAM_BASE_ADDRESS + address);
    address += 1;
    rd_data ++;
  }
}
